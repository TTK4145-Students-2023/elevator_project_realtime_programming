...
func DialBroadcastUDP(port int) net.PacketConn {
	...
	syscall.SetsockoptInt(s, syscall.SOL_SOCKET, syscall.REUSEADDR, 1)
	if err != nil {
		fmt.Println("Error: SetSockOpt REUSEADDR:", err)
	}
	syscall.SetsockoptInt(s, syscall.SOL_SOCKET, syscall.BROADCAST, 1)
	if err != nil {
		fmt.Println("Error: SetSockOpt BROADCAST:", err)
	}
	syscall.SetsockoptInt(s, syscall.SOL_SOCKET, syscall.REUSEPORT, 1)
	if err != nil {
