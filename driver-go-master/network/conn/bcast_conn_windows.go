...
func DialBroadcastUDP(port int) net.PacketConn {
	config := &net.ListenConfig{Control: func(network, address string, conn syscall.RawConn) error {
		return conn.Control(func(descriptor uintptr) {
			syscall.SetsockoptInt(syscall.Handle(descriptor), syscall.SOL_SOCKET, syscall.REUSEADDR, 1)
			syscall.SetsockoptInt(syscall.Handle(descriptor), syscall.SOL_SOCKET, syscall.BROADCAST, 1)
		})
	},
	}
